...
- **Diagnostic state endpoint**: `StateServer` serves a full live JSON snapshot at `http://127.0.0.1:<port>/state` (channels: levels/routes/mute/gains/clarity/`refCorr`; outputs: mode/strength/stable/reference + winner; plus `referenceInput`). **Opt-in** via `AUDIOMIXER_STATE` (a port number, else default 7077). Read-only, loopback only. `MainViewModel.BuildStateJson` marshals to the UI thread. This is the fastest way to watch the automixer's *reasoning* (env vs corr vs the selected leader) without the GUI.
